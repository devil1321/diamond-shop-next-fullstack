  ...
  return (
    <div className='favoruites-form m-4 mr-5 md:m-0 rounded-[25px] text-white h-[600px] rounded-[25px  ] p-2 md:py-12 md:px-6 md:w-1/3'>
      ...
      <div ref={sliderMinRef} onTouchMove={(e)=>handleMin(e)} onTouchStart={()=>setIsMin(true)} onTouchEnd={()=>setIsMin(false)} onMouseDown={()=>setIsMin(true)} onMouseUp={()=>setIsMin(false)} className="favoruites-form-field mb-12  relative top-0 left-0">
        <input className='accent-green-300' onMouseMove={(e)=>handleMin(e)} type="range" value={min} min={0} max={500} onChange={(e)=>setMin(Number(e.target.value))} />
        <div className="p-2 bg-figma-green absolute top-5 left-0 rounded-md font-bold" ref={minRef}></div>
      ...
      <div ref={sliderMaxRef} onTouchMove={(e)=>handleMax(e)} onTouchStart={()=>setIsMax(true)} onTouchEnd={()=>setIsMax(false)} onMouseDown={()=>setIsMax(true)} onMouseUp={()=>setIsMax(false)} className="favoruites-form-field mb-12 relative top-0 left-0">
        <input className='accent-purple-400' onMouseMove={(e)=>handleMax(e)} type="range" value={max} min={0} max={500} onChange={(e)=>setMax(Number(e.target.value))} />
        <div className="p-2 bg-figma-purple absolute top-5 left-0 rounded-md font-bold" ref={maxRef}></div>
      </div>
      <h2 className="text-2xl font-bold mb-6">Color</h2>
      <div className="favoruites-colors mb-12 flex gap-3">
        {colors.map((c:any) => <div onClick={(e)=>handleColor(e)} style={{backgroundColor:`#${c.hex}`}} className='favoruites-color cursor-pointer h-5 w-5 border-[1px] border-transparent rounded-full'></div>)}
      </div>
      <button className='block hover:opacity-50 w-[100%] py-2 mt-[90px] font-bold text-white rounded-md'>Filter</button>
      <Link href="/cart">
